...
def construct_input(input_shape):
    ...
    img_metas=[dict(box_type_3d=LiDARInstance3DBoxes, 
                        pad_shape=[(*input_shape, 3) for _ in range(12)], 
                        img_shape=[(*input_shape, 3) for _ in range(12)],
                        extrinsics=np.random.rand(12, 4, 4),
                        intrinsics=np.random.rand(12, 3, 3),
                        timestamp=np.random.rand(2, 6)
                        )]
    
    input = dict(img_inputs=
        torch.ones(()).new_empty(
                (1, 12, 3, *input_shape)).cuda(),
        img_metas=img_metas
       )
    return input
