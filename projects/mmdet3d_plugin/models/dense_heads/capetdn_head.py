...
class CAPETemporalDNHead(nn.Module):
    """refer to the DETR transformer head.
    ...
    def __init__(self,
                 ...
                 transformer=dict(
                        ...
                        att_layer=dict(type='CrossViewAttention',
                            ...
                            dim_head= 32,
                            conditional=True)),
                 temporal_fusion_layer=dict(type='TemporalCrossAttention',
                        hidden_dim=512,
                        num_queries=900,
                        qkv_bias=True,
                        heads=4,
        ...
        self.embed_dims = transformer['att_layer'].get('hidden_dim', 512)

        self.num_pred = transformer['num_layers']
    ...
    def _get_camera_parameters(self, img_metas):
        ...
        for img_meta in img_metas:
            ...
            # print('########'*10, img_meta.keys())
            
            for i in range(len(img_meta['extrinsics'])):
                extrinsic.append(img_meta['extrinsics'][i].T) # see the definition in dataset!!!!
        ...
        extrinsics = torch.from_numpy(extrinsics) # (B, N, 4, 4)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        extrinsics, intrinsics = extrinsics.to(x.device), intrinsics.to(x.device)
        R = extrinsics[:, :, :3, :3]
        R_inv = torch.inverse(R)
        t = R_inv @ extrinsics[:, :, :3, 3:4]
        I_inv = torch.inverse(intrinsics[:, :, :3, :3])   
        ...
        if not self.with_prev_aux_loss:
            ...
            outs_dec = torch.nan_to_num(outs_dec)
            prev_out_dec = torch.nan_to_num(prev_out_dec)
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            if self.with_prev_aux_loss:
                ...
                prev_tmp[..., 0:2] = prev_tmp[..., 0:2].sigmoid()
                prev_tmp[..., 4:5] += reference[..., 2:3]
                prev_tmp[..., 4:5] = prev_tmp[..., 4:5].sigmoid()
                if self.with_time:
        ...
        if self.with_prev_aux_loss:
            prev_all_cls_scores = torch.stack(prev_outputs_classes)
            prev_all_bbox_preds = torch.stack(prev_outputs_coords)
    ...
    def prepare_for_loss(self, mask_dict):
        ...
        if len(output_known_class) > 0:
            ...
            if self.with_prev_aux_loss:
                ...
                prev_output_known_class = prev_output_known_class.permute(1, 2, 0, 3)[(bid, map_known_indice)].permute(1, 0, 2)
                prev_output_known_coord = prev_output_known_coord.permute(1, 2, 0, 3)[(bid, map_known_indice)].permute(1, 0, 2)
    ...
    def loss_single_with_prev_loss(self,
        ...
        # classification loss
        cls_scores = cls_scores.reshape(-1, self.cls_out_channels)
        prev_cls_scores = prev_cls_scores.reshape(-1, self.cls_out_channels)
        ...
        # calculate prev frame cls loss
        prev_loss_cls = self.prev_aux_loss_weight * self.loss_cls(
            prev_cls_scores, labels, label_weights, avg_factor=cls_avg_factor)
        ...
        # regression L1 loss
        bbox_preds = bbox_preds.reshape(-1, bbox_preds.size(-1))
        prev_bbox_preds = prev_bbox_preds.reshape(-1, prev_bbox_preds.size(-1))

        normalized_bbox_targets = normalize_bbox(bbox_targets, self.pc_range)
        isnotnan = torch.isfinite(normalized_bbox_targets).all(dim=-1)
        bbox_weights = bbox_weights * self.code_weights

        prev_normalized_bbox_targets = normalized_bbox_targets.clone()
        time = time[:, None].repeat(1, prev_normalized_bbox_targets.shape[0]//num_imgs).view(-1)
        prev_normalized_bbox_targets[:, 0] -= time * prev_normalized_bbox_targets[:, 8]
        prev_normalized_bbox_targets[:, 1] -= time * prev_normalized_bbox_targets[:, 9]

        loss_bbox = self.loss_bbox(
                bbox_preds[isnotnan, :10], normalized_bbox_targets[isnotnan, :10], bbox_weights[isnotnan, :10], avg_factor=num_total_pos)
        
        isnotnan = torch.isfinite(prev_normalized_bbox_targets).all(dim=-1)

        # calculate prev frame reg loss
        prev_loss_bbox = self.prev_aux_loss_weight * self.loss_bbox(
                prev_bbox_preds[isnotnan, :2], prev_normalized_bbox_targets[isnotnan, :2], bbox_weights[isnotnan, :2], avg_factor=num_total_pos) 

        loss_cls += prev_loss_cls
        loss_bbox += prev_loss_bbox

        loss_cls = torch.nan_to_num(loss_cls)
        loss_bbox = torch.nan_to_num(loss_bbox)
        return loss_cls, loss_bbox

    def dn_loss_single_with_prev_loss(self,
                    cls_scores,
                    bbox_preds,
                    prev_cls_scores,
                    prev_bbox_preds,
        ...
        prev_loss_cls = self.prev_aux_loss_weight * self.loss_cls(
            prev_cls_scores, known_labels.long(), label_weights, avg_factor=cls_avg_factor)
        ...
        prev_normalized_bbox_targets = normalized_bbox_targets.clone()
        bs = time.shape[0]
        time = time[:, None].repeat(1, prev_normalized_bbox_targets.shape[0]//bs).view(-1)
        if time.shape[0] < prev_normalized_bbox_targets.shape[0]:
            time = torch.cat([time, time[-1][None]], 0)
        prev_normalized_bbox_targets[:, 0] -= time * prev_normalized_bbox_targets[:, 8]
        prev_normalized_bbox_targets[:, 1] -= time * prev_normalized_bbox_targets[:, 9]
    ...
    def loss(self,
        ...
        if self.with_prev_aux_loss:
            prev_all_cls_scores = preds_dicts['prev_all_cls_scores']
            prev_all_preds_scores = preds_dicts['prev_all_bbox_preds']
            time = preds_dicts['mean_time_stamp']
        ...
        if not self.with_prev_aux_loss:
            ...
            time_list = [
                time for _ in range(num_dec_layers)
            ]
            losses_cls, losses_bbox = multi_apply(
                self.loss_single_with_prev_loss, all_cls_scores, all_bbox_preds,
                prev_all_cls_scores, prev_all_preds_scores, time_list,
                all_gt_bboxes_list, all_gt_labels_list, 
                all_gt_bboxes_ignore_list)
        ...
        if preds_dicts['dn_mask_dict'] is not None:
            if self.with_prev_aux_loss:
                known_labels, known_bboxs, output_known_class, output_known_coord, prev_output_known_class, prev_output_known_coord, num_tgt = self.prepare_for_loss(preds_dicts['dn_mask_dict'])
            else:
            ...
            if self.with_prev_aux_loss:
                dn_losses_cls, dn_losses_bbox = multi_apply(
                    self.dn_loss_single_with_prev_loss, output_known_class, output_known_coord, 
                    prev_output_known_class, prev_output_known_coord, time_list,
